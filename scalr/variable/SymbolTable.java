 ...
 * {@linkplain Function}'s ASTs. The actual management of symbol tables is handled by {@linkplain Function}s. A symbol
 * table is nothing more than a <code>{@linkplain HashMap}&lt;{@linkplain String},
...
public final class SymbolTable
{
	...
	/**
	 * The symbol table that is currently being used to store and retrieve variables. This should point to some symbol
	 * table (<code>{@linkplain HashMap}&lt;{@linkplain String}, {@linkplain Expression}&gt;</code>) before the
	 * <code>addVar()</code> and <code>getVar()</code> methods are called.
	 */
	public static HashMap<String, Expression>		currentSymbolTable;

	/**
	 * Currently not used externally, but it contains the {@linkplain Function}s currently on the stack, but not the
	 * link to the line where they are called. This is to be added soon, when the grammar provides meaningful errors.
	 */
	 ...
	 * list of statements at the time it is added. To account for this discrepancy, {@linkplain FunctionReference}s are
	 * used by the grammar, to postpone retrieval of the actual {@linkplain Function} object until it is called by the
	 * <code>getValue()</code> method, which must occur after the function has been defined.
	 * 
	 * @param f The {@linkplain Function} to add. The name is retrieved from the {@linkplain Function} itself.
	 * @throws FunctionExistsError If there is already a function of the specified name. As of now, all functions are
	 *         global.
	 */
	 ...
	 * {@linkplain Function}, allowing for recursion. It contains a blank symbol table, and its statements are fresh (in
	 * the sense that they are not modified by the <code>getValue()</code> method).
	 * 
	 * @param id The {@linkplain String} representing the name of the {@linkplain Function} to retrieve.
	 * @return The {@linkplain Function} corresponding to the given name. Throws a
	 *         {@linkplain FunctionDoesNotExistException} if there is no {@linkplain Function} with the given name.
	 */
	...
	/**
	 * Adds the given {@linkplain Expression} with the given {@linkplain String} ID to the current symbol table. If it
	 * already exists, it will be replaced, so long as the type is correct.
	 * 
	 * @param var The {@linkplain String} ID of the variable to add to the current symbol table.
	 * @param value The {@linkplain Expression} to map this variable to. If the variable exists, its type should not
	 *        change.
	 * @return True if this variable doesn't exist in the current symbol table. In the case that the variable already
	 *         exists in the symbol table and the replacement variable is of the same type, false.
	 */
	...
	/**
	 * Retrieves a {@linkplain Variable} from the current symbol table. If it does not exist, a
	 * {@linkplain NullReferenceException} is thrown.
	 * 
