 ...
 * they must ensure that no new variables appear in the current symbol table. Aside from that, this class simply
 * evaluates a series of {@linkplain Expression}s as long as some {@linkplain Expression} of
...
public class WhileStatement implements Expression
{
	...
	/**
	 * {@linkplain WhileStatement}s are constructed using the {@linkplain Expression} to check, since that is the first
	 * piece of information seen while parsing. Also, {@linkplain WhileStatement}s, at least on the backend, need not
	 * have any statements to be executable.
	 * 
	 * @param expr The {@linkplain Expression} to check each iteration of this {@linkplain WhileStatement}.
	 */
	 ...
	 * permitted.
	 * 
	 * @param expr Any {@linkplain ExpressionType} is valid. It is only added if the {@linkplain Expression} is not
	 *        null.
	 */
	...
	/**
	 * Executes each of the statements contained by this {@linkplain WhileStatement} so long as the given
	 * {@linkplain Expression} is true. It records the variables in the current symbol table before execution, and then
	 * removes them after each time the statements are executed. It also checks if the user wants to cancel out of the
	 * loop, or if they are returning a value.
	 * 
	 ...
	 * {@linkplain ControlOperation} that is a <code>return</code> {@link Expression}, so all {@linkplain Expression}s
	 * that care about returns should be wary.
	 * 
