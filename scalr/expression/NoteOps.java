...
/**
 * This class handles all modifications to {@linkplain Note}s. Since a {@linkplain Note} cannot be necessarily modified
 * at the time that it is parsed. It is rather complex in its evaluation, but it simply decodes what the parser parsed
 * and executes the appropriate modification. Long chains of note modifications (note.length().pitch()...) are
 * represented as long chains of {@linkplain NoteOps} {@linkplain Expression}s.
 */
public class NoteOps implements Expression
{
	 ...
	 * modification is the first thing that is known, {@linkplain NoteOps} are constructed using this central piece of
	 * information.
	 * 
	 * @param type A {@linkplain String} corresponding to the type of modification to the {@linkplain Note}. It can be
	 *        "pit" (pitch), "vol" (volume), or "len" (length).
	 */
	...
	/**
	 * Adds the note to perform this modification to. The grammar automatically chains a series of {@linkplain NoteOps}
	 * starting with a {@linkplain Note} or {@linkplain VariableReference}, however any expression of
	 * <code>{@linkplain ExpressionType}.NOTE</code> will suffice.
	 * 
	 ...
	 * mod.
	 * 
	 * @param s A {@linkplain String} indicating that the parameter of the {@linkplain Note} is to be modded, and not
	 *        set. If it is "+", then the value is to be increased. If it is "-", then the value is to be decreased.
	 */
	...
	/**
	 * Adds the number to mod the {@linkplain Note} by or set the {@linkplain Note}'s parameter to. The
	 * {@linkplain Expression} should be of <code>{@link ExpressionType}.NUMBER</code>.
	 * 
	...
	/**
	 * Serves a similar purpose as addPitch. Indicates to this {@linkplain NoteOps} that the {@linkplain Length} is to
	 * be set absolutely. This method handles both fractions (1, 1/2, etc.) as well as the {@linkplain Length} names.
	 * 
	 ...
	 *        indicating the position in the {@linkplain Scale}, starting from 1, to access the {@linkplain Degree}.
	 */
	...
	/**
	 * Evaluates this {@linkplain NoteOps}. This method is essentially a ton of if statements to determine which fields
	 * were set, and thus perform the appropriate operation. The grammar produces valid and deterministic cases, but it
	 * may be tough to unit test. Regardless, if the syntax is valid, this method always returns an
	 * {@linkplain Expression} of type {@linkplain Note}.
	 * 
	 ...
	 * 
	 * @return A {@linkplain String} displaying the {@linkplain Note} operation as the SCALR programmer may have written
	 *         it.
	 */
	@Override
